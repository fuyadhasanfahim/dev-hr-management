...
import type { RecipientSendStatus } from "@/redux/features/receipt/receiptApi";

export interface ReceiptEmailDialogProps {
  open: boolean;
  onClose: () => void;
  /** Mongo `_id` of the client that owns this receipt. */
  clientId: string;
 ...
 * should receive a payment receipt. Mirrors QuotationEmailDialog exactly.
 */
export function ReceiptEmailDialog({
  ...
  onSend,
  isSending = false,
}: ReceiptEmailDialogProps) {
  const { data: clientEmails, isLoading } = useGetClientEmailsQuery(clientId, {
    skip: !clientId || !open,
  });
  ...
  const options = useMemo<EmailOption[]>(() => {
    ...
    (clientEmails || []).forEach((e: ClientEmail) => {
      ...
      const isPrimary =
        !primaryAssigned && e.type === "client" && /\(Primary\)/i.test(e.label || "");
      ...
      map.set(key, {
        email,
        label: e.label || email,
        type: isPrimary ? "primary" : e.type === "team_member" ? "team_member" : "client",
      });
    });
  ...
  const toggle = (email: string) => {
    ...
    setSelected((prev) =>
      prev.includes(email) ? prev.filter((e) => e !== email) : Array.from(new Set([...prev, email])),
    );
  };
  ...
  const successCount = hasResults ? results!.filter((r) => r.status === "sent").length : 0;
  const failedCount = hasResults ? results!.filter((r) => r.status === "failed").length : 0;
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="receipt-email-dialog-title"
      aria-busy={inFlight}
    >
      <button
        type="button"
      ...
      <div className="relative w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl border bg-card shadow-2xl">
        <div className="flex items-start justify-between gap-4 p-5 border-b">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 id="receipt-email-dialog-title" className="text-base font-semibold truncate">
                {hasResults ? "Delivery summary" : "Send receipt to client"}
              </h2>
              <p className="text-xs text-muted-foreground mt-0.5 truncate">
                {receiptLabel
                  ? hasResults
                    ? `Results for ${receiptLabel}`
                    : `Recipients for ${receiptLabel}`
        ...
        <div className="p-5">
          {hasResults ? (
            <ResultsView results={results!} successCount={successCount} failedCount={failedCount} />
          ) : isLoading ? (
            ...
            <div className="rounded-xl border border-dashed p-6 text-center">
              <p className="text-sm font-medium">No emails available for this client.</p>
              <p className="mt-1 text-xs text-muted-foreground">
                Add at least one email to the client profile or team members first.
              </p>
            </div>
          ) : (
            <fieldset
  ...
  return (
    <label
      className={[
        ...
        isPrimary && !checked
          ? "border-teal-200 dark:border-teal-900/50 bg-teal-50/30 dark:bg-teal-950/10"
          : "",
      ].join(" ")}
    >
      <input
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-sm font-medium truncate">{opt.email}</span>
          {isPrimary && (
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {!ok && r.error && (
                  <p className="text-xs text-red-700/90 mt-0.5 wrap-break-word">{r.error}</p>
                )}
              </div>
            </li>
          );
...
export default ReceiptEmailDialog;
