        ...
        <Card className="h-full flex flex-col shadow-sm hover:shadow-md transition-shadow border-muted/50 overflow-hidden">
                ...
                {hasActiveTasks && (
                    <Link href="/tasks" passHref>
                        <Button
                            variant="ghost"
                            size="sm"
                            className="text-[10px] text-primary font-bold uppercase tracking-wider h-6 px-2 gap-1 hover:bg-primary/10 rounded-full"
                        >
            ...
            <CardContent className="p-0 flex-1 flex flex-col justify-between">
                        ...
                        {activeTasks.slice(0, 4).map((task: any) => {
                            ...
                            return (
                                <Link
                                    href="/tasks"
                                    key={task._id}
                                    className="group block relative transition-all hover:bg-muted/30"
                                >
                                    {/* Priority Indicator Line */}
                                    <div
                                        className={`absolute left-0 top-0 bottom-0 w-[3px] ${getPriorityStyles(task.priority)} opacity-70 transition-all group-hover:w-[4px] group-hover:opacity-100`}
                                    />
                                    ...
                                    <div className="pl-4 pr-4 py-3 flex flex-col gap-1">
                                        <div className="flex items-start justify-between w-full gap-2">
                                            ...
                                            <div
                                                className={`shrink-0 flex items-center gap-1 text-[10px] font-bold tracking-tight whitespace-nowrap ${isOverdue ? "text-red-600 dark:text-red-400" : "text-muted-foreground"}`}
                                            >
                                        ...
                                        <div className="flex items-center justify-between text-[10px] font-medium text-muted-foreground">
                                            <div className="flex items-center gap-1.5 truncate">
                                                <span className="font-bold text-[9px] text-foreground/60 uppercase">
                                                    #
                                                    {task.orderId
                                                        ?.orderNumber || "N/A"}
                                                </span>
                                            ...
                                            <div className="flex items-center gap-1 shrink-0">
                                                {task.status ===
                                                    ...
                                                    <Badge
                                                        variant="outline"
                                                        className="bg-amber-100/50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-300 text-[9px] font-bold h-4 px-1.5"
                                                    >
                                                        Review
                                                    </Badge>
                                                )}
                ...
                {activeTasks.length > 4 && (
                    <Link
                        href="/tasks"
                        className="block w-full py-2.5 border-t border-muted/30 text-center text-[10px] font-bold tracking-wide text-muted-foreground hover:bg-muted/30 hover:text-primary transition-colors bg-muted/5 mt-auto"
                    >
