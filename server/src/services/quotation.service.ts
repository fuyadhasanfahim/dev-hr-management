...
export class QuotationService {
  ...
  static async getQuotationById(id: string): Promise<IQuotation | null> {
    return await QuotationModel.findById(id).populate(
      "clientId",
      "name clientId emails",
    );
  }
  ...
  static async convertToOrder(id: string, userId: string): Promise<any> {
    console.log("CONVERTING QUOTATION:", id, "BY USER:", userId);
    ...
    const orderPayload = {
      ...
      createdBy: userId.length === 24 ? new Types.ObjectId(userId) : userId as any,
    };

    const order = await OrderServices.createOrderInDB(orderPayload);

    // Save orderId to quotation
    quotation.orderId = order?._id as Types.ObjectId;
