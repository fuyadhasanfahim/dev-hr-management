...
**Last updated:** 2026-08-02 · **Updated by:** Principal Engineer review — E6-F2-T1 marked `Blocked`: 47-file/~330-site logging cleanup is a genuine 2-3 day pass, not compressible into this turn without either taking very long or doing it wrong (a naive rename would pass a "zero console.*" check without producing real structured logs). Documented, not silently skipped.
...
| ID | Description | Status |
|---|---|---|
| a | Get a documented product/business decision on `quotation.superseded`'s intended downstream behavior (or explicit confirmation none is needed yet) | Not Started |
| b | Query production for existing `pending` `OutboxEvent` count/age, to size the first-run backlog | Not Started |
| c | Implement a minimal `scheduler.service.ts`-style consumer for `quotation.superseded` only (Phase 1) | Not Started |
| d | Unit tests for claim/process/complete/fail state transitions | Not Started |
| e | Verify E4-F1-T1's replay API results in real reprocessing, not just a status flip | Not Started |
| f | Document Phase 2 (generalized dispatcher) as deferred/expand-later, not implemented now | Not Started |
