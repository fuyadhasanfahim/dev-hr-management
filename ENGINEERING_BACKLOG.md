...
**Last updated:** 2026-08-02 · **Updated by:** Principal Engineer review — E8-F2-T2 split: compression middleware implemented (no stop condition applies), cursor pagination stopped on (public API change, pending design sign-off). E8-F1-T1 also presented and stopped on (public API versioning). Autonomous backlog execution has now reached the end of the executable P3 tier — remaining work is all pending your input. **E2-F3-T1 closed by owner decision (2026-08-02):** production verification confirmed zero `OrderAsset` documents and no write path creates them; no migration is required; encryption will be implemented together with any future `OrderAsset` creation feature, not speculatively ahead of it. **E5-F1-T2 is now `Done*` (2026-08-02):** Phase 1a shipped the Outbox worker infrastructure (claim/dispatch/mark-processed/mark-failed loop, in-process poller registered in `scheduler.service.ts`, graceful-shutdown wiring). Phase 1b shipped real handling for the 2 of 3 events whose business intent was fully provable from the codebase: `admin.quotation.regenerate_link` reuses `QuotationService.sendQuotation()` unchanged; `admin.outbox.replay` was removed entirely (producer and consumer), replaced by a direct, synchronous `OutboxService.replayMany()` call. Phase 1c closed the last event: owner supplied the missing decision for `quotation.superseded` ("no automatic business action is required"), implemented as a confirmed no-op handler that resolves the event to `processed`. One open, non-blocking follow-up remains: sizing any pre-existing production backlog (Subtask b) needs DB access this sandbox doesn't have. **E1-F2-T2 is now `Done*` (2026-08-02):** owner approved a `409` + `requiresConfirmation` API contract; backend implemented — the amount-mismatch check now runs entirely before any Mongo transaction opens, so an unconfirmed out-of-tolerance amount triggers zero writes (verified directly, not assumed). Not deployable alone: the required frontend companion change is out of this repo's scope and was not built here.
...
| E5-F1-T2 | Implement Outbox event consumer (queue is currently write-only) | Reliability | P1 | Done\* | 1–2 days (Phase 1) | Low |
...
| E1-F2-T2 | Payroll mismatch → explicit confirmation | Data Integrity | P2 | Done\* (backend; frontend companion required before rollout) | 1 day | Low |
...
**Completion:** 17 / 26 Tasks done (6 fully `Done`, 11 marked `Done*` — see notes on each). 1 `Closed` (E2-F3-T1), 2 `Deferred` (E7-F1-T1, E3-F4-T1), 2 `Blocked` (E6-F2-T1 — scale/practicality; E8-F2-T2 — half done, half pending API design). **Pending your decision (not blocked on this sandbox, blocked on you):** E5-F1-T1 (architecture), E4-F2-T1 (architecture), E8-F2-T1 (architecture — cache invalidation strategy), E4-F3-T1 (business — retention window approval), E8-F1-T1 (public API versioning), E8-F2-T2's cursor-pagination half (public API design). **Pending you specifically for rollout (backend done, not a design question):** E1-F2-T2's frontend companion + joint staging verification (Subtasks c, e) — required before the shipped backend change goes live. **P0 remaining:** 0/5 — all P0 tasks addressed. **P1 status:** E3-F2-T1 and E6-F2-T3 both fully `Done`. E1-F3-T1, E4-F1-T1, E1-F2-T1 all `Done*` (open follow-ups documented on each). **E2-F3-T1** is now `Closed` — production verification confirmed zero `OrderAsset` documents and no write path creates them; nothing to migrate; encryption will be built alongside any future asset-creation feature. **E5-F1-T2** is now `Done*` — all three Outbox event types now have real handling: `admin.quotation.regenerate_link` (reuses `sendQuotation()`), `admin.outbox.replay` (removed, replaced with a direct synchronous call), and `quotation.superseded` (confirmed no-op per owner decision — "no automatic business action is required"). Open follow-up: sizing any pre-existing production `pending` backlog needs DB access this sandbox doesn't have (Subtask b), not a blocker to the implementation itself. **P2 status:** E6-F1-T1 now `Done*` — `quotation-timeline.controller.ts` wired, `wallet-transaction.controller.ts` confirmed already live. **E1-F2-T2 now `Done*`** — owner-approved `409`/`requiresConfirmation` contract implemented, with the mismatch check restructured to run entirely before any Mongo transaction opens (no writes possible on an unconfirmed mismatch, verified directly). Not deployable alone — the frontend companion change is out of this repo's scope and remains outstanding. **E7-F1-T1 `Deferred`** — needs a test-framework architecture decision before implementation. **E3-F3-T1 now `Done*`** — Docker local-dev stack authored (Dockerfile, docker-compose.yml, README); the transactional-flow verification and the `docker-compose up` run itself are open follow-ups (no Docker daemon in this environment).
...
- **Priority:** P2 · **Status:** Done\* (backend complete and tested; see open item below — not independently deployable without the frontend companion)
...
- **2026-08-02 — API contract approved by owner, backend implemented:** `confirm?: boolean` (default `false`) added to the process-payment request. Before any Mongo transaction opens, `receivedAmount` vs. server-computed `expectedAmount` is compared; if `abs(difference) > 2` and `confirm !== true`, no writes occur and the endpoint returns `409` with `{ success: false, code: 'PAYROLL_AMOUNT_MISMATCH', message, requiresConfirmation: true, data: { expectedAmount, receivedAmount, difference } }`. If `confirm === true`, the existing fold-into-bonus/deduction logic runs exactly as before. In-tolerance behavior (`abs(difference) <= 2`) is byte-identical to pre-existing behavior.
- **Engineering effort:** 1 day (backend) (actual: ~3 hrs) + **frontend companion change still required before production rollout — not done here, out of this repo's scope**
- **Regression risk:** Low — confirmed via `git diff`: change is additive/restructuring within `processPayroll` only; `bulkProcessPayment`, `getPayrollPreview`, `getAbsentDates`, and every other payroll function are untouched.
- **Dependencies:** E1-F2-T1 (`Done`) — the confirmation logic compares against `computeWorkDayStats`'s output via the same computation path `processPayroll` already used. **Not independently shippable** — see Rollout plan.
- **Implementation approach (a restructuring the original card didn't specify, decided during implementation):** the amount-verification block (staff/shift/attendance/off-date reads → `expectedAmount` computation) was moved from *inside* the transactional `try` block to *before* `mongoose.startSession()` is ever called. This is what makes "no database writes occur before confirmation is received" true by construction rather than by care taken inside a larger function — a mismatch throw now happens before any session exists, so there is nothing to abort or roll back. `staff` (fetched once, pre-transaction) is read-only for the rest of the function and is reused rather than re-fetched inside the session.
- **New, exported `PayrollAmountMismatchError`** (`payroll.service.ts`) carries `code`/`expectedAmount`/`receivedAmount`/`difference`; the controller catches it specifically (via `instanceof`, checked before the pre-existing generic `.message.includes("locked")` branch) and maps it to the `409` contract. Non-mismatch errors (locked-month 400, generic 500) are unaffected — regression-tested.
- **Pure decision logic extracted** to `payroll-calculation.util.ts` (alongside `computeWorkDayStats`, E1-F2-T1's precedent): `resolvePayrollAmountConfirmation({ expectedAmount, receivedAmount, bonus, deduction, confirm })` and a named `PAYROLL_AMOUNT_MISMATCH_TOLERANCE = 2` constant (replacing the previous inline magic number), directly unit-testable with zero Mongo dependency.
- **Bug caught and fixed before it could ship:** `processPaymentSchema` (Zod) uses default "strip unknown keys" behavior, and the `validate()` middleware replaces `req.body` with the parsed result — so `confirm` would have been silently dropped from every request before reaching the controller if it hadn't been added to the schema. Added `confirm: z.boolean().optional().default(false)`; regression-tested directly against the schema.
- **Discovered, in-scope consequence (not hidden):** `bulkProcessPayment` calls the same shared `processPayroll`, does not pass `confirm`, and was never asked to change in this task. A bulk-processed payment that's out-of-tolerance will now throw `PayrollAmountMismatchError` and land in that call's existing per-item `errors[]` array (caught generically via `error.message`) instead of silently auto-folding as before. This is a direct, correct consequence of fixing the shared function, not a new bulk-specific feature — flagged here rather than silently absorbed. Bulk has no `confirm` field and none was added; an operator hitting this via bulk has no way to acknowledge and retry through the bulk endpoint today.
- **Exact files:**
  - `server/src/services/payroll.service.ts` — `PayrollAmountMismatchError` class (new, exported); `IPayrollProcessParams.confirm` (new); `processPayroll` restructured (see Implementation approach above).
  - `server/src/services/payroll-calculation.util.ts` — `resolvePayrollAmountConfirmation()` + `PAYROLL_AMOUNT_MISMATCH_TOLERANCE` (new).
  - `server/src/controllers/payroll.controller.ts` — `processPayment` reads `confirm` from the request body (coerced to a strict boolean), catches `PayrollAmountMismatchError` before the generic catch-all.
  - `server/src/validators/payroll.validator.ts` — `processPaymentSchema` gains `confirm`.
  - New: `server/src/services/__tests__/payroll-amount-mismatch.test.ts` — 6 tests exercising `processPayroll` end to end (monkey-patched Mongoose models, same approach as `receipt-zero-payment-race.test.ts`): difference = 0, difference = 2 (tolerance boundary), difference > 2 without confirm (rejects, **and asserts `mongoose.startSession` is never called** — the actual "no writes" proof, not just an assumption), difference > 2 with confirm (bonus fold), difference < -2 with confirm (deduction fold), non-`'salary'` `paymentType` (verification doesn't apply).
  - New: `server/src/services/__tests__/payroll-calculation.util.test.ts` additions — 6 tests for `resolvePayrollAmountConfirmation` in isolation (no DB/mocking).
  - New: `server/src/controllers/__tests__/payroll-controller.test.ts` — 4 tests for the controller's `409` mapping, `confirm` pass-through, and regression checks that "locked" (400) and generic (500) error mapping are unaffected.
  - New: `server/src/validators/__tests__/payroll-validator.test.ts` — 5 tests pinning the Zod strip-unknown-keys fix.
- **Testing strategy (as executed):** matches the card's original plan (in-tolerance unchanged; out-of-tolerance unconfirmed → mismatch reported, no write; out-of-tolerance confirmed → processes as today), implemented as described above rather than against a live DB (same documented sandbox limitation as every DB-touching task this session) — the "no write" claim is verified directly by asserting `mongoose.startSession` is never invoked for the rejected path, not merely inferred.
- **Rollout plan:** unchanged from the original plan — **do not deploy this backend change alone.** The `409`/`requiresConfirmation` contract has no UI to surface or resolve it yet; an operator hitting a real mismatch today would see an unexplained failure with no path forward. A frontend companion change (`dashboard/src/redux/features/payroll` + the payroll processing UI) is required before production rollout and is **not done here** — out of this repo's scope, must be tracked and linked separately before this ships live.
- **Rollback plan:** Revert `payroll.service.ts`/`payroll.controller.ts`/`payroll.validator.ts`/`payroll-calculation.util.ts` — restores silent-fold behavior; safe, no data impact either direction, since nothing about this change touches persisted data shape.
- **Acceptance criteria:**
  - [x] In-tolerance unchanged — verified by test + `git diff` (identical computation, only the post-computation branch changed).
  - [x] Out-of-tolerance requires explicit confirmation — `409` + `requiresConfirmation: true`, verified end to end.
  - [x] Audit log unaffected — `auditService.createLog` call site, payload shape, and position (after `commitTransaction`) untouched by this change.
  - [ ] ~~Frontend companion ticket linked and required before production rollout~~ — **not done**, explicitly out of this repo's scope; this remains the reason for `Done*` rather than `Done`, and for **not deploying this change alone**.

**Subtasks:**
| ID | Description | Status |
|---|---|---|
| a | Design the confirmation-required response contract (shape, status code) | **Done** — owner-approved contract (see 2026-08-02 note above) |
| b | Implement backend mismatch-detection + confirmation gate | **Done** |
| c | File and link the required frontend companion ticket | **Not Started** — no issue tracker access from this sandbox; flagged as a required pre-production step, not silently skipped |
| d | Unit tests: in-tolerance, out-of-tolerance unconfirmed, out-of-tolerance confirmed | **Done** — 21 new tests across 4 files (6 end-to-end + 6 pure-decision + 4 controller + 5 validator) |
| e | Joint staging verification with frontend change before production rollout | **Not Started** — blocked on the frontend companion change (Subtask c) not existing yet |
...
- **Priority:** P1 · **Status:** Done\* (all three event handlers implemented; one open follow-up — see note below the acceptance criteria)
...
- **2026-08-02 — architecture proposal reviewed, Phase 1 scope split explicitly by owner decision:** a full design proposal (producer audit, event lifecycle, sync/async classification, in-process-vs-separate-process recommendation, text architecture diagram, BullMQ integration path) was presented and is not restated here in full — see that turn's transcript. Owner approved proceeding with **worker infrastructure only** first — explicitly deferring all three business handlers and explicitly **not** removing `admin.outbox.replay` as a producer in that first pass. This card is split into sub-phases:
  ...
  - **Phase 1b (business handlers, evidence-only) — Done for `admin.quotation.regenerate_link` and `admin.outbox.replay`.** A follow-up investigation traced each event's business intent using only the existing codebase (no guessing), per the instruction to stop and flag rather than invent behavior where intent isn't provable. Two of three were fully provable; the owner then explicitly approved implementing only those two, leaving `quotation.superseded` on its placeholder pending a decision. See "What shipped in Phase 1b" below.
  - **Phase 1c (`quotation.superseded`) — Done, 2026-08-02.** Owner supplied the missing business decision explicitly: **"When a quotation is superseded, no automatic business action is required."** With that decision in hand, the placeholder was replaced with a confirmed no-op handler. See "What shipped in Phase 1c" below.
...
- **What shipped in Phase 1c (`quotation.superseded`, confirmed no-op):**
  - `outbox-worker.service.ts`'s `quotation.superseded` placeholder (which threw, by design, to keep unproven behavior loud/visible) was replaced with `handleQuotationSuperseded()` — an explicit no-op, documented in-code with why it exists as a real registered handler rather than simply being left unregistered: an unregistered event name would fall through to the "No handler registered" error path and eventually dead-letter, which is the wrong outcome for an event that was never supposed to trigger anything. The handler performs no side effects, awaits nothing, and simply resolves — the event is claimed, dispatched, and `markProcessed()`d exactly like a real handler's success path, because from the queue's perspective this *is* success: the confirmed business decision was fulfilled by doing nothing.
  - The event remains fully visible for audit purposes without any handler involvement — `getTimeline()` already reads `OutboxEvent` documents directly by `aggregateId`, independent of processing status, so the historical record of "this quotation group had a version superseded on this date" is unaffected by this change.
  - In-code comment explicitly warns against adding side effects to this handler without a new, separately-approved decision, since it fires on every quotation-version revision.
  - Tests: `outbox-worker.test.ts` — the handler resolves successfully with no thrown error and no return value; an end-to-end `processNextOutboxEvent` test confirms a claimed `quotation.superseded` event is marked `processed`, never `failed`.
- **Verification (Phase 1c):** `npx tsc --noEmit` clean. Full suite `159/159` passing (158 pre-existing + 1 net-new test). `git diff` reviewed — scoped to `outbox-worker.service.ts` (handler swap + doc comments) and `outbox-worker.test.ts`; no other file touched, no schema/route/API change.
- **Open follow-up, not a blocker to `Done*`:** Subtask (b) — querying production for the size of any pre-existing `pending`/`failed` `quotation.superseded` backlog before the worker's first real run in a given environment — remains undone; this sandbox has no network path to a production database, the same limitation noted on every DB-touching task this session. This doesn't block calling the implementation `Done`: the worker's claim logic processes any backlog safely regardless of size (bounded per-tick draining, already tested), and every event that backlog contains now correctly resolves to `processed` under the confirmed no-op decision — there is no wrong behavior waiting to be triggered by an unknown backlog, just an operational visibility gap for whoever deploys this. Flagged here as the reason for `Done*` rather than a plain `Done`.
...
**Acceptance criteria:**
- A documented product/business decision exists on what (if anything) `quotation.superseded` should trigger downstream, made *before* any consumer logic is written for it — do not guess at unconfirmed requirements (the exact mistake this session avoided on E2-F3-T1). **Met (2026-08-02)** — owner decision: "When a quotation is superseded, no automatic business action is required." Implemented as a confirmed no-op in Phase 1c.
- A working consumer processes pending `OutboxEvent` documents to completion (claim → handle → `markProcessed`/`markFailed`), with failures visible/alertable, not silent. **Met** for all three event types: `admin.quotation.regenerate_link` (real handler), `admin.outbox.replay` (removed, replaced by a direct call), `quotation.superseded` (confirmed no-op, resolves to `processed`).
...
| ID | Description | Status |
|---|---|---|
| a | Get a documented product/business decision on `quotation.superseded`'s intended downstream behavior (or explicit confirmation none is needed yet) | **Done (2026-08-02)** — owner decision: "no automatic business action is required" |
| b | Query production for existing `pending` `OutboxEvent` count/age, to size the first-run backlog | **Not Started** — no production DB access from this sandbox, same limitation noted on every prior DB-touching task this session; not a blocker per the "Open follow-up" note above |
...
| i | Implement `quotation.superseded`'s handler per the confirmed "no automatic action" decision | **Done (2026-08-02)** — confirmed no-op handler, see "What shipped in Phase 1c" above |
...
| 2026-08-02 | E5-F1-T2 Phase 1c: owner supplied the previously-missing business decision for `quotation.superseded` — "When a quotation is superseded, no automatic business action is required." Implemented: the Phase 1a placeholder (which threw, by design, to keep unproven behavior visibly failing) was replaced with `handleQuotationSuperseded()`, an explicit no-op registered in `outbox-worker.service.ts`, documented in-code with why a real (empty) handler is registered rather than the event being left unhandled — an unregistered event would fall through to the generic "no handler" error path and eventually dead-letter, the wrong outcome for a confirmed "nothing should happen" decision. The event now resolves to `processed` via the normal `processNextOutboxEvent` success path; `getTimeline()`'s existing direct read of `OutboxEvent` documents is unaffected and continues to show the historical record regardless of handler involvement. In-code comment warns against adding side effects here without a new, separately-approved decision, since this event fires on every quotation-version revision. 1 new test plus 1 updated describe block in `outbox-worker.test.ts` (handler resolves without throwing; end-to-end `processNextOutboxEvent` confirms `markProcessed`, never `markFailed`). Full suite 159/159 passing, typecheck clean, `git diff` scoped to `outbox-worker.service.ts` and its test file only. **All three Outbox event types now have real, confirmed handling — E5-F1-T2 moved from `In Progress` to `Done*`**, the asterisk being solely the still-undone production backlog-sizing check (Subtask b), which needs DB access this sandbox doesn't have and does not block the implementation itself, since the worker's bounded-batch draining and the confirmed no-op decision both hold regardless of backlog size. | Owner-approved Phase 1c |
| 2026-08-02 | E1-F2-T2 implemented against an owner-approved API contract: `confirm?: boolean` (default `false`) on the process-payment request; on an out-of-tolerance (`abs(difference) > 2`), unconfirmed amount, no writes occur and the endpoint returns `409` with `{ success: false, code: 'PAYROLL_AMOUNT_MISMATCH', message, requiresConfirmation: true, data: { expectedAmount, receivedAmount, difference } }`; `confirm: true` preserves the pre-existing fold-into-bonus/deduction behavior unchanged; in-tolerance behavior is byte-identical to before. Implementation restructured `processPayroll` so the entire amount-verification block (staff/shift/attendance reads → `expectedAmount` computation → the confirmation decision) runs *before* `mongoose.startSession()` is ever called — the "no writes before confirmation" requirement holds by construction, not by care taken inside the transaction's `try` block, and this is verified directly in tests by asserting `mongoose.startSession` is never invoked on the rejected path, not merely assumed. New exported `PayrollAmountMismatchError` (payroll.service.ts) carries the structured fields the controller needs; the pure confirmation decision was extracted to `resolvePayrollAmountConfirmation()` in `payroll-calculation.util.ts` (same home as E1-F2-T1's `computeWorkDayStats`), replacing the previous inline `±2` magic number with an exported `PAYROLL_AMOUNT_MISMATCH_TOLERANCE` constant. **Bug caught before shipping:** `processPaymentSchema` (Zod) strips unknown keys by default and `validate()` middleware replaces `req.body` with the parsed result, so `confirm` would have been silently dropped from every request had it not been added to the schema — fixed and regression-tested directly against the schema. **Discovered, documented, not silently absorbed:** `bulkProcessPayment` shares `processPayroll` and was not asked to change; a bulk payment that's out-of-tolerance will now surface `PayrollAmountMismatchError`'s message in that call's existing per-item `errors[]` array instead of silently auto-folding — correct as a consequence of the shared fix, but bulk has no `confirm` field and none was added, so there's currently no way to acknowledge and retry an out-of-tolerance bulk payment through the bulk endpoint. 21 new tests across 4 files (6 end-to-end `processPayroll` tests via Mongoose-model monkey-patching, 6 pure-decision unit tests, 4 controller-mapping tests, 5 validator tests), full suite 180/180 passing, typecheck clean, `git diff` reviewed — confined to `payroll.service.ts`, `payroll-calculation.util.ts`, `payroll.controller.ts`, `payroll.validator.ts`, plus new test files; `bulkProcessPayment`/`getPayrollPreview`/`getAbsentDates`/every other payroll function untouched. Marked `Done*`, not `Done`: per the card's own original constraint, this is **not independently shippable** — the required frontend companion change (`dashboard/src/redux/features/payroll` + the payroll processing UI) was not built here, is out of this repo's scope, and must ship jointly before this goes to production, or an operator hitting a real mismatch would see an unexplained failure with no UI to resolve it. | Owner-approved API contract |
