...
| E5-F1-T2 | Implement Outbox event consumer (queue is currently write-only) | Reliability | P1 | In Progress (worker infra shipped, business handlers not yet approved) | 1–2 days (Phase 1) | Low |
...
**Completion:** 15 / 26 Tasks done (6 fully `Done`, 9 marked `Done*` — see notes on each). 1 `Closed` (E2-F3-T1), 2 `Deferred` (E7-F1-T1, E3-F4-T1), 2 `Blocked` (E6-F2-T1 — scale/practicality; E8-F2-T2 — half done, half pending API design). **Pending your decision (not blocked on this sandbox, blocked on you):** E5-F1-T2 (product decision), E5-F1-T1 (architecture), E1-F2-T2 (public API contract + frontend coordination), E4-F2-T1 (architecture), E8-F2-T1 (architecture — cache invalidation strategy), E4-F3-T1 (business — retention window approval), E8-F1-T1 (public API versioning), E8-F2-T2's cursor-pagination half (public API design). **Every remaining `Not Started` task in the backlog now requires your input — the autonomous pass is complete for this session.** **P0 remaining:** 0/5 — all P0 tasks addressed. **P1 status:** E3-F2-T1 and E6-F2-T3 both fully `Done`. E1-F3-T1, E4-F1-T1, E1-F2-T1 all `Done*` (open follow-ups documented on each). **E2-F3-T1** is now `Closed` — production verification confirmed zero `OrderAsset` documents and no write path creates them; nothing to migrate; encryption will be built alongside any future asset-creation feature. **E5-F1-T2** is now `In Progress` — worker infrastructure (Phase 1a) shipped; business handlers (Phase 1b) remain blocked on the same product decision as before (see its Subtask a). **P2 status:** E6-F1-T1 now `Done*` — `quotation-timeline.controller.ts` wired, `wallet-transaction.controller.ts` confirmed already live. **E7-F1-T1 `Deferred`** — needs a test-framework architecture decision before implementation. **E3-F3-T1 now `Done*`** — Docker local-dev stack authored (Dockerfile, docker-compose.yml, README); the transactional-flow verification and the `docker-compose up` run itself are open follow-ups (no Docker daemon in this environment).
...
- **Priority:** P1 · **Status:** In Progress — **Phase 1a done (worker infrastructure), Phase 1b not started (business handlers require separate approval)**
...
- **2026-08-02 — architecture proposal reviewed, Phase 1 scope split explicitly by owner decision:** a full design proposal (producer audit, event lifecycle, sync/async classification, in-process-vs-separate-process recommendation, text architecture diagram, BullMQ integration path) was presented and is not restated here in full — see that turn's transcript. Owner approved proceeding with **worker infrastructure only** this pass — explicitly deferring all three business handlers (`quotation.superseded`, `admin.quotation.regenerate_link`) and explicitly **not** removing `admin.outbox.replay` as a producer (the design proposal's recommendation to drop it as an event is still just a recommendation, not actioned). This card is therefore split into two sub-phases going forward:
  - **Phase 1a (worker infrastructure) — Done, this pass.** See "What shipped" below.
  - **Phase 1b (business handlers) — Not Started, requires separate approval per event type before implementation**, per the still-open questions from the design proposal (does `quotation.superseded` notify internal staff or the client? should `admin.outbox.replay` be removed as an event, per the design recommendation, or kept?).
- **What shipped in Phase 1a (worker infrastructure only, no business logic):**
  ...
  - **Placeholder handlers registered for all three currently-produced event types** (`quotation.superseded`, `admin.quotation.regenerate_link`, `admin.outbox.replay` — the latter kept, per explicit instruction not to remove it this pass). Each placeholder **throws** rather than silently succeeding, so claimed events go through the existing retry/backoff/dead-letter path exactly as a real failing handler would, instead of being falsely marked `processed` with no actual effect. This is a deliberate choice: it keeps the dispatch path exercised end-to-end today, while being honest that no business outcome has actually happened yet — visible in the admin `/outbox` UI as `dead_letter` after `maxAttempts`, not silently invisible.
  - `server/src/services/scheduler.service.ts` — registers `startOutboxWorker()` in `startAllSchedulers()` alongside the existing 6 jobs, same pattern. `stopAllSchedulers()` changed from sync to `async` (previously synchronous, zero behavior change for its other 6 jobs — `clearInterval` calls are unchanged) so it can `await stopOutboxWorker()`, letting an in-flight claim finish instead of being abandoned.
  - `server/src/server.ts` — **pre-existing gap fixed as a direct consequence of "respect graceful shutdown" being in this task's explicit scope:** `stopAllSchedulers()` was previously never called anywhere, including on `SIGTERM`/`SIGINT` — `createGracefulShutdown`'s `closeResources` only closed the Mongo connection. Without wiring it in, "graceful shutdown" could not be respected for the Outbox worker (or any other scheduler) at all. `closeResources` now calls `await schedulerService.stopAllSchedulers()` before `mongoose.connection.close()`. This benefits all 6 pre-existing scheduler jobs too, not just the new Outbox worker — flagged here as a side effect of closing a gap this task's own acceptance requirement ("respect graceful shutdown") could not be met without.
  - New: `server/src/services/__tests__/outbox-worker.test.ts` — 14 tests (empty-queue no-op, successful-handler marks-processed, missing-handler marks-failed, handler-throws marks-failed with message, non-`Error` throw still stringified, multi-event drain, `maxEvents` cap respected, `shouldStop()` checked before and mid-drain, and all three placeholder handlers confirmed to reject with a clear "not implemented yet" message rather than resolving silently).
  - **Explicitly not touched:** no business behavior for any event type; `admin.outbox.replay` producer left in place, unremoved; no route, controller, or public API contract changed; `quotation.service.ts`/`quotation-timeline.controller.ts` untouched.
- **Verification:** `npx tsc --noEmit` clean. Full suite `151/151` passing (137 pre-existing + 14 new). `git diff` reviewed — scoped to `constants/timing.ts` (+1 constant), `server.ts` (`closeResources` body), `scheduler.service.ts` (registration + async signature), plus the two new files; no producer call site, route, or controller modified.
...
**Acceptance criteria:**
- A documented product/business decision exists on what (if anything) `quotation.superseded` should trigger downstream, made *before* any consumer logic is written for it — do not guess at unconfirmed requirements (the exact mistake this session avoided on E2-F3-T1).
- A working consumer processes pending `OutboxEvent` documents to completion (claim → handle → `markProcessed`/`markFailed`), with failures visible/alertable, not silent.
- An admin replay via the existing E4-F1-T1 API results in the event actually being reprocessed end-to-end, not just its status field flipping.
- Zero behavior change to any of the 3 existing producer call sites.
- E4-F1-T1's and (if wired by then) E6-F1-T1's replay/regenerate-link actions are confirmed functional end-to-end, or explicitly documented as still-intentionally-deferred if the product decision above defers them.
...
| e | Verify E4-F1-T1's replay API results in real reprocessing, not just a status flip | **Not Started** — depends on Phase 1b (a real handler must exist before "reprocessing" has an observable effect to verify) |
